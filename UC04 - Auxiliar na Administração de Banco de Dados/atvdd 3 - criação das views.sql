...
***/
CREATE VIEW view_vendas_dinheiro AS SELECT v.`data`, iv.subtotal AS total_produto, iv.nome_produto, iv.quantidade, iv.valor_unitario, c.nome, c.cpf, c.telefone FROM venda v
INNER JOIN item_venda iv ON iv.venda_id = v.id 
INNER JOIN cliente c ON c.id = v.cliente_id
WHERE v.tipo_pagamento = 'D' ORDER BY `data` DESC;

/*SELECT * FROM view_vendas_dinheiro;*/
...
***/
CREATE VIEW view_vendas_fabricante AS SELECT p.nome, p.descricao, p.estoque, p.fabricante, iv.quantidade, v.`data` FROM produto p
INNER JOIN item_venda iv ON iv.produto_id = p.id
INNER JOIN venda v ON v.id = iv.venda_id
WHERE p.fabricante LIKE '%lar%' 
ORDER BY p.nome ASC;

/*SELECT * FROM view_vendas_fabricante;*/
...
***/
CREATE VIEW view_produto_cliente AS SELECT c.nome AS cliente, c.cpf, c.telefone, c.endereco, p.nome, p.descricao, p.estoque, p.fabricante, SUM(iv.subtotal), SUM(iv.quantidade) FROM cliente c 
